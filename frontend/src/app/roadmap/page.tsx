...
export default function DiscoveryPage() {
  ...
  const generatePath = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:80'}/api/roadmap/generate`, {
        method: "POST",
  ...
  const initializePath = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:80'}/api/roadmap/activate`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
