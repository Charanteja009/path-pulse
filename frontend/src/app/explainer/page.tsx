...
export default function RepoExplainer() {
    ...
    const fetchHistory = async () => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:80'}/api/repo/history/${userId}`);
            if (res.ok) {
  ...
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:80'}/api/repo/analyze`, {
      method: "POST",
