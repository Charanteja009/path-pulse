...
export default function ExecutionView() {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:80'}/api/roadmap/single/${id}`)
      .then(res => res.json())
      .then(result => {
        setData(result);
  ...
  const toggleStep = async (stepTitle: string) => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:80'}/api/roadmap/update-progress`, {
      method: "PATCH",
